...
from utils import *
seed = 42
KERNEL_PATH = '/z/abwilf/hffn/glorot_kernel.pk'
tf.random.set_seed(seed), tf.compat.v1.set_random_seed(seed), np.random.seed(seed)
save_pk(KERNEL_PATH, tf.keras.initializers.GlorotUniform(seed=seed))
KER = load_pk(KERNEL_PATH)
...
class ABS_LSTM4(Layer):
    ...
    def call(self, inputs):
       # s = K.zeros((K.shape(inputs)[0],self.units))
        ...
        s = K.zeros((batch_size, self.units))
